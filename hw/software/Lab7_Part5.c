...
volatile int *Key1Ptr = (int *)KEY1_BASE; // Key1 address
volatile int *LEDPtr = (int *)LEDS_BASE; // LED base address

uint8 *RAM8bitPtr = (uint8 *)INFERRED_RAM_BE_0_BASE; // RAM base address
uint16 *RAM16bitPtr = (uint16 *)INFERRED_RAM_BE_0_BASE; // RAM base address
uint32 *RAM32bitPtr = (uint32 *)INFERRED_RAM_BE_0_BASE; // RAM base address
...
void pushbutton_isr(void *context)
{
	...
	int key_value;
	key_value = *(Key1Ptr + 3);//read the pushbutton interrupt register
	*(Key1Ptr + 3) = key_value; // clear the pushbutton interrupt

		// BIT MASKING
	if (key_value ==(key_value & 0x2)) //ensures key 1 was pressed
	{
		*LEDPtr = 0x55; //unique pattern
		jtag_display((uint8*)"RAM TEST DONE", 13);
		testdone();
	}
		 return;
}
...
int RAM32bit(uint32 *startaddress, unsigned int numBytestoTest,uint32 testData)
{
	...
	for ( i = 0; i < numBytestoTest;)
	{
		if ( testData != *(startaddress + i)) //this is a failure
	  			{
					//Address: 0x????_???? Read:  0x????_???? Expected: 0x????_????"
					//jtag_display((uint8*)"ERROR: Address: 0x????_???? Read:  0x????_???? Expected: 0x????_????", 68);
					//jtag_display((uint8*)"ERROR: Address: %d Read: %d Expected: %d",startaddress,testData, *(startaddress + i), 68);				
					printf("\n\nERROR: Address: %u Read: %u Expected: %u",i,*(startaddress + i),testData );
					*LEDPtr = 0xFF; //turn on LEDS
...
void testdone (void) //to stop the program when key one is pressed
{
	while(1);
}
...
alt_ic_isr_register(KEY1_IRQ_INTERRUPT_CONTROLLER_ID,
					KEY1_IRQ,pushbutton_isr,0,0);
*(Key1Ptr + 2) = 0xF;//write to the pushbutton interrupt mask register, and set 3 mask bits to 1 (bit 0 is Nios II reset) */

/*
 create a ramp test that will call the 32-bit function with a ramp pattern
(0x12345678). The ramp test data should be a 32-bit word. 
*/
  while (1)
  {
	  //numBytestoTest is 4096 because there are 4096 registers
	RAM8bit(RAM8bitPtr, 4096, 0x00);
	...
	/* key_value = *(Key1Ptr);
	 if (key_value ==(key_value & 0x2)) //if KEY1 was pressed
	 {                                 
                *LEDPtr = 0x55;    
				jtag_display((uint8*)"RAM TEST DONE", 13);
     } */
